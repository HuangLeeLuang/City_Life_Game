...
- Exact filenames are `assets/images/event-<id>-desktop.webp` and `assets/images/event-<id>-mobile.webp` for every ID in the preceding list. Their corrected scenes are, in the same order: chase shell casings and a police street seal; an indebted traffic clerk's plate records; a car-wash witness and campaign vehicle; Old Six's broad asset offering; the police chief detaining Mira's brother; street meal versus formal dinner; the developer lawyer's ledger settlement; an unregistered police car with blood and city permit; Kael's precision-driving lesson; the reporter's footage of Azhe leaving the armored truck; physical training; three temporary job choices; guns, fake plates, and a family surveillance photo; a livestreamed chase with crowds reporting its location; Old Six's corruption ledger; cafe/bar relaxation; helping residents while gathering information; injury recovery; the retired-name search order on a modified radio; casino promotion with Azhe's voice leaking through a call; Azhe's abandoned bar and drain tunnel; supervised range training; the closed-trunk sedan delivery; and Mira's garage repair-or-coffee meeting.
...
### 2026-08-08 story and event choice batch

Built-in ImageGen produced 84 distinct choice masters for the `mainline`, `character`, and `event` categories. Every final is a stripped quality-82 WebP at 1536x1024 (3:2), stored as `assets/images/choices/<parent-id>--<option-id>.webp` and mapped through `CHOICE_ART` with action-specific Traditional Chinese alternative text.

- Coverage: 28 mainline choices, 6 Difei character choices, and 50 general-event choices.
- Prompt family: each prompt included the live event title, summary, option label, detail, and available result; requested the concrete chosen action at its decisive moment; and retained the realistic modern East Asian harbor crime-noir palette of deep blue-green, concrete gray, restrained amber, and mint practical light.
